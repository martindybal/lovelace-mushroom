        ...
        return html`
            <div class="actions" ?rtl=${rtl}>
                <mushroom-button-group .fill=${layout !== "horizontal"} ?rtl=${rtl}>
                    ${this._custom_buttons.map(this.renderCustomButton)}
                </mushroom-button-group>            
            </div>
        `;
    }
    
    renderFirstButton(): TemplateResult {
        var button = this._custom_buttons[0];

        return this.renderCustomButton(button);
    }

    renderCustomButton(button: Button): TemplateResult {
        console.log('renderCustomButton');
        console.log(button);
        return html`
            <mushroom-button @click=${button.tap_action}>
